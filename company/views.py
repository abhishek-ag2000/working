...
def get_company_object(request, company_pk):
    ...
    response['Content-Disposition'] = 'attachment; filename={}-{}.json'.format(company.name, dte.now())
    return response
...
def get_company_excel(request, company_pk):
    ...
    response['Content-Disposition'] = 'attachment; filename={}.xls'.format(company.name)
    ...
    columns_st = ['Stock_Name', 'Voucher_Id', 'GST_Rate', 'Quantity', 'Rate', 'Opening', 'Group_Name', 'HSN', 'Unit']
    ...
    companys = companys.extra(select={'datestr': "to_char(created_date, 'DD-MM-YYYY')", 'datestr1': "to_char(modified_date, 'DD-MM-YYYY')"})
    ...
    rows_g = LedgerGroup.objects.filter(company=company).exclude(group_base__name__contains='Primary').values_list('group_name', 'id', 'Master__group_Name', 'base__name', 'base__is_debit')
    ...
    rows_j = JournalVoucher.objects.filter(company=company).values_list('Date', 'voucher_id', 'voucher_type', 'By__ledger_name', 'To__ledger_name', 'Debit', 'Credit')
    ...
    rows_p = PaymentVoucherRows.objects.filter(payment__company=company.pk).values_list('payment__date', 'payment__id', 'payment__account__ledger_name', 'particular__ledger_name', 'amount')
    ...
    rows_r = ReceiptVoucherRows.objects.filter(receipt__company=company.pk).values_list('receipt__date', 'receipt__id', 'receipt__account__ledger_name', 'particular__ledger_name', 'amount')
    ...
    rows_c = ContraVoucherRows.objects.filter(contra__company=company.pk).values_list('contra__date', 'contra__id', 'contra__account__ledger_name', 'particular__ledger_name', 'amount')
    ...
    rows_sg = StockGroup.objects.filter(company=company).values_list('name', 'id', 'under__name')
    ...
    rows_sm = SimpleUnit.objects.filter(company=company).values_list('symbol', 'id', 'formal')
    ...
    rows_cu = CompoundUnit.objects.filter(company=company).values_list('firstunit', 'id', 'conversion', 'seconds_unit')
    ...
    rows_st = StockItem.objects.filter(company=company).values_list('stock_name', 'id', 'gst_rate', 'Quantity', 'rate', 'opening', 'under__name', 'hsn', 'simple_unit__symbol')
    ...
    rows_ss = StockBalance.objects.filter(company=company).values_list('stockitem__stock_name', 'stockitem__id', 'opening_stock', 'closing_quantity', 'closing_stock')
...
class OrganisationCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        counter = Organisation.objects.filter(user=self.request.user).count() + 1
    ...
    def get_context_data(self, **kwargs):
        ...
        context['legal_product'] = ProductActivated.objects.filter(user=self.request.user, product__id=10, is_active=True)
        return context
...
class OrganisationListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            context['Products_CRM'] = ProductActivated.objects.filter(
                user=self.request.user, product__id=7, is_active=True)
...
class OrganisationDetailsView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super(OrganisationDetailsView, self).get_context_data(**kwargs)
        company = self.get_object()
        context['static_page_details'] = StaticPage.objects.filter(organisation=company.pk).first()
...
class CompanyDetailView(DetailView, ProductExistsRequiredMixin, LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        company = self.get_object()  # get_object_or_404(Company, pk=self.kwargs['company_pk'])
        context['legal_product'] = ProductActivated.objects.filter(user=self.request.user, product__id=10, is_active=True)
        context['company'] = company
        ...
        capital_clo = company.company_ledger_group.filter(group_base__name__contains='Capital Account').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        context['capital_ac_total'] = capital_clo

        sundeb_clo = company.company_ledger_group.filter(group_base__name__contains='Sundry Debtors').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        context['total_dues_to_collect'] = sundeb_clo

        suncre_clo = company.company_ledger_group.filter(group_base__name__contains='Sundry Creditors').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        context['total_dues_to_pay'] = suncre_clo

        fxdast_clo = company.company_ledger_group.filter(group_base__name__contains='Fixed Assets').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']

        curntast_clo = company.company_ledger_group.filter(group_base__name__contains='Current Assets').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        ...
        context['total_current_asset'] = curntast_clo + qs2

        total_asset = abs(fxdast_clo) + abs(curntast_clo) + abs(qs2)

        curntlia_clo = company.company_ledger_group.filter(group_base__name__contains='Current Liabilities').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        context['total_current_liabilities'] = curntlia_clo

        context['working_capital'] = context['total_current_asset'] - context['total_current_liabilities']

        closing_stock = 0  # company.Company_stock_journal.aggregate(the_sum=Coalesce(Sum('closing_stock'), Value(0)))['the_sum']
        opening_stock = 0  # company.Company_stock_journal.aggregate(the_sum=Coalesce(Sum('opening_stock'), Value(0)))['the_sum']

        total_purchase = company.company_ledger_group.filter(group_base__name__contains='Purchase Accounts').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        total_sales = company.company_ledger_group.filter(group_base__name__contains='Sales Account').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        total_directexp = company.company_ledger_group.filter(group_base__name__contains='Direct Expenses').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        total_directinc = company.company_ledger_group.filter(group_base__name__contains='Direct Incomes').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        total_indirectexp = company.company_ledger_group.filter(group_base__name__contains='Indirect Expense').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        total_indirectinc = company.company_ledger_group.filter(group_base__name__contains='Indirect Income').aggregate(the_sum=Coalesce(Sum('group_ledger__closing_balance'), Value(0)))['the_sum']
        ...
        company.pl = np
        company.capital = context['capital_ac_total']
        company.asset = total_asset
        company.save()
        ...
        investment = context['capital_ac_total'] + company.pl

        if investment or investment != 0:
            context['roi'] = (company.pl / investment) * 100
        else:
            context['roi'] = 0

        if context['working_capital'] or context['working_capital'] != 0:
            context['return_working_capital'] = (company.pl / int(context['working_capital'])) * 100
        else:
            context['return_working_capital'] = 0

        # pl_ledger = company.Companys.get(ledger_name__exact='Profit & Loss A/c')

        # total_debitob = 0
        # total_creditob = 0

        #opening_balance = abs(pl_ledger.opening_balance) + abs(total_creditob) - abs(total_debitob)
        top_ledgers_deb = company.company_ledger_master.filter(ledger_group__group_base__name__exact='Sundry Debtors')
        context['top_ledger_sales'] = top_ledgers_deb.annotate(total=Coalesce(Sum('party_ledger_sales__sub_total'), 0)).order_by('-total')[:10]  # all ledger sales total
        context['total_ledger'] = context['top_ledger_sales'].aggregate(partial_total=Sum('total'))['partial_total']

        top_ledgers_cre = company.company_ledger_master.filter(ledger_group__group_base__name__exact='Sundry Creditors')
        context['top_ledger_purchase'] = top_ledgers_cre.annotate(total=Coalesce(Sum('party_ledger_purchase__sub_total'), 0)).order_by('-total')[:10]  # all ledger purchase total
        ...
        context['top_stock_total'] = context['top_stock'].aggregate(partial_total=Sum('total_sales'))['partial_total']
        ...
        context['stock_margin'] = context['top_stock'].annotate(
            ...
            Avg_sales=Case(
                When(quantity_sales__gt=0, then=F('total_sales') / F('quantity_sales')),
                default=None,
                output_field=FloatField()
            )
        )
        ...
        while date_cursor <= period_selected.end_date:
            month_partial_total = result.filter(
                ...
                voucher_date__year=date_cursor.year).aggregate(partial_total=Sum('total'))['partial_total']

            month_partial_total_purchase = result_purchase.filter(
                voucher_date__month=date_cursor.month,
                voucher_date__year=date_cursor.year).aggregate(partial_total=Sum('total'))['partial_total']

            results[date_cursor.month, date_cursor.year] = [month_partial_total, month_partial_total_purchase]
...
def organisation_delete_ajax(request, organisation_pk):
    ...
    if request.method == "POST":
        ...
        data['organisation_list'] = render_to_string('organisation/organisation_list_2.html', context)
    else:
        context = {'organisation': organisation}
        data['html_form'] = render_to_string('organisation/organisation_confirm_delete.html', context, request=request)
...
def specific_company_details(request, company_pk):
    ...
    if not request.user.is_authenticated:
        ...
        legal_product = ProductActivated.objects.filter(product__id=10, is_active=True)
    else:
        ...
        legal_product = ProductActivated.objects.filter(user=request.user, product__id=10, is_active=True)
...
def search_auditors(request, company_pk):
    ...
    if query:
        result = user_profile.filter(Q(user__username__icontains=query) | Q(email__icontains=query) | Q(full_name__icontains=query)).exclude(name=request.user)
    else:
...
def search_accountant(request, company_pk):
    ...
    if query:
        result = Profile.objects.filter(Q(user__username__icontains=query) | Q(email__icontains=query) | Q(full_name__icontains=query)).exclude(name=request.user)
    else:
        result = Profile.objects.none()
...
class StaticPageCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            context['portfolios'] = PortfolioForm_formset(self.request.POST,self.request.FILES)
            context['teams'] = TeamForm_formset(self.request.POST,self.request.FILES)
        else:
...
def live_page(request,organisation_pk,period_selected_pk):
    ...
    organisation = get_object_or_404(Organisation, pk=organisation_pk)
    static_page_details= get_object_or_404(StaticPage, organisation=organisation)
    service_details= Service.objects.filter(staticpage=static_page_details.pk)
    portfolio_details= Portfolio.objects.filter(staticpage=static_page_details.pk)
...
class StaticPageUpdateView(UpdateView):
    ...
    def get_object(self):
        ...
        static_page_return = get_object_or_404(StaticPage, organisation=organisation)
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST or self.request.FILES:
            context['services'] = ServiceForm_formset(self.request.POST,instance=static_page)
            context['portfolios'] = PortfolioForm_formset(self.request.POST,self.request.FILES,instance=static_page)
            context['teams'] = TeamForm_formset(self.request.POST,self.request.FILES,instance=static_page)
        else:
