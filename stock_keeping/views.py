...
from django.db.models import Value, Sum, Count, F, Q, ExpressionWrapper
...
def profit_and_loss_view(request, pk, period_selected_pk):
    ...
    # if ldse == 0 or ldse:
    #     total = - ldsecr
    # elif ldsecr == 0 or ldsecr:
    #     total = ldse
    # else:
    #     total = ldse - ldsecr

    # Indirect Income #credit
    ldi = LedgerGroup.objects.filter(company=company.pk, group_Name__iexact='Indirect Incomes')
    ...
    # company_detail = Company.objects.get(company_pk=company.pk)
    ...
    return render(request, 'stock_keeping/P&L.html', context)


@login_required
@product_1_activation
def trial_balance_view(request, pk, period_selected_pk):
    company = get_object_or_404(Company, pk=pk)
    period_selected = get_object_or_404(PeriodSelected, pk=period_selected_pk)
    ...
    grp_debit = LedgerGroup.objects.filter(company=company.pk, Master__group_Name__iexact='Primary', balance_nature__iexact='Debit')
    grp_credit = LedgerGroup.objects.filter(company=company.pk, Master__group_Name__iexact='Primary', balance_nature__iexact='Credit')

    grp_debit_co = LedgerGroup.objects.filter(company=company.pk, Master__group_Name__iexact='Primary', balance_nature__iexact='Debit')
    grp_credit_co = LedgerGroup.objects.filter(company=company.pk, Master__group_Name__iexact='Primary', balance_nature__iexact='Credit')
    ...
    ledger_pl = LedgerMaster.objects.get(company=company.pk, ledger_name__iexact="Profit & Loss A/c")
    ...
    gs_opening = LedgerGroup.objects.filter(company=company.pk, Master__group_Name__iexact='Primary')
    ...
    gs = LedgerGroup.objects.filter(company=company.pk, Master__group_Name__iexact='Primary').exclude(Q(group_Name__iexact="Stock-in-hand") | Q(group_Name__iexact="Sales Account") | Q(
        group_Name__iexact="Purchase Accounts") | Q(group_Name__iexact="Indirect Expenses") | Q(group_Name__iexact="Indirect Incomes") | Q(group_Name__iexact="Direct Incomes") | Q(group_Name__iexact="Direct Expenses"))
    ...
    # purchases
    gs_purchase = LedgerGroup.objects.filter(company=company.pk, group_Name__iexact="Purchase Accounts")
    gs_purchase_total = gs_purchase.aggregate(the_sum=Coalesce(Sum('ledgergroups__Closing_balance'), Value(0)))['the_sum']

    # sales
    gs_sales = LedgerGroup.objects.filter(company=company.pk, group_Name__iexact="Sales Account")
    gs_sales_total = gs_sales.aggregate(the_sum=Coalesce(Sum('ledgergroups__Closing_balance'), Value(0)))['the_sum']

    # Direct Expense
    gs_directexp = LedgerGroup.objects.filter(company=company.pk, group_Name__iexact="Direct Expenses")
    gs_directexp_total = gs_directexp.aggregate(the_sum=Coalesce(Sum('ledgergroups__Closing_balance'), Value(0)))['the_sum']

    # Direct Income
    gs_directinc = LedgerGroup.objects.filter(company=company.pk, group_Name__iexact="Direct Incomes")
    gs_directinc_total = gs_directinc.aggregate(the_sum=Coalesce(Sum('ledgergroups__Closing_balance'), Value(0)))['the_sum']

    # Indirect Expense
    gs_indirectexp = LedgerGroup.objects.filter(company=company.pk, group_Name__iexact="Indirect Expenses")
    gs_indirectexp_total = gs_indirectexp.aggregate(the_sum=Coalesce(Sum('ledgergroups__Closing_balance'), Value(0)))['the_sum']

    # Indirect Income
    gs_indirectinc = LedgerGroup.objects.filter(company=company.pk, group_Name__iexact="Indirect Incomes")
    ...
    return render(request, 'stock_keeping/Trial_Balance/trial_bal.html', context)


@login_required
@product_1_activation
def balance_sheet_view(request, pk, period_selected_pk):
    company = get_object_or_404(Company, pk=pk)
    period_selected = get_object_or_404(PeriodSelected, pk=period_selected_pk)

    # All primary groups with nature of group is 'Liabilities'
    group_lia = LedgerGroup.objects.filter(company=company.pk, Master__group_Name__iexact='Primary', Nature_of_LedgerGroup__iexact='Liabilities')
    ...
    group_ast = LedgerGroup.objects.filter(company=company.pk, Master__group_Name__iexact='Primary', Nature_of_LedgerGroup__iexact='Assets')
    ...
    ca_group = LedgerGroup.objects.get(company=company.pk, group_Name__iexact='Current Assets')
    total_ca = (ca_group.positive_closing - ca_group.negative_closing) + abs(qs2)
    ...
    ldsecr = lde.aggregate(the_sum=Coalesce(Sum('negative_closing'), Value(0)))['the_sum']
    if ldse == 0 or not ldse:
        total = - ldsecr
    elif ldsecr == 0 or not ldsecr:
        total = ldse
    else:
        total = ldse - ldsecr
    ...
    if total_lia_positive or total_ast_negative:
        if total_ca < 0:
            if total_pl >= 0:
                total_liabilities_1 = abs(total_lia_positive) + abs(total_ast_negative) + abs(total_pl) + abs(qs2)
            else:
                total_liabilities_1 = abs(total_lia_positive) + abs(total_ast_negative)
        else:
            if total_pl >= 0:
                total_liabilities_1 = abs(total_lia_positive) + abs(total_pl) + abs(total_ast_negative)
            else:
                total_liabilities_1 = abs(total_lia_positive) + abs(total_ast_negative) + abs(qs2)
    ...
    if total_lia_negative or total_ast_positive:
        if total_ca > 0:
            if total_pl < 0:
                total_asset_1 = abs(total_lia_negative) + abs(total_ast_positive) + abs(total_pl)
            else:
                total_asset_1 = abs(total_lia_negative) + abs(total_ast_positive) + abs(qs2)
        else:
            if total_pl < 0:
                total_asset_1 = abs(total_lia_negative) + abs(total_ast_positive) + abs(total_pl) + abs(qs2)
            else:
                total_asset_1 = abs(total_lia_negative) + abs(total_ast_positive)
    else:
        total_asset_1 = 0
    ...
    return render(request, 'stock_keeping/balance_sheet.html', context)
