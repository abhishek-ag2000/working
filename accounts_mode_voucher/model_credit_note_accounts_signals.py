...
@receiver(pre_save, sender=CreditNoteAccountsVoucher)
...
def update_gst_totals_credit_note(sender, instance, *args, **kwargs):
    ...
    total_cgst_extra = instance.credit_note_term_accounts.aggregate(
        the_sum=Coalesce(Sum('cgst_total'), Value(0)))['the_sum']
    total_sgst_extra = instance.credit_note_term_accounts.aggregate(
        the_sum=Coalesce(Sum('sgst_total'), Value(0)))['the_sum']
    total_igst_extra = instance.credit_note_term_accounts.aggregate(
...
def update_total_tax_credit_note(sender, instance, *args, **kwargs):
    ...
    total_tax_extra = instance.credit_note_term_accounts.aggregate(
...
#     extra_total = instance.credit_note_term_accounts.aggregate(
#         the_sum=Coalesce(Sum('total'), Value(0)))['the_sum']
...
# @receiver(post_save, sender=CreditNoteAccountsVoucher)
# @prevent_signal_call_on_bulk_load
...
@receiver(post_save, sender=CreditNoteAccountsTerm)
@prevent_signal_call_on_bulk_load
...
@receiver(post_save, sender=CreditNoteAccountTax)
@prevent_signal_call_on_bulk_load
...
@receiver(pre_delete, sender=CreditNoteAccountsVoucher)
def delete_journal_voucher_against_terms_credit_note(sender, instance, **kwargs):
    ...
    """
    credit_extra = CreditNoteAccountsTerm.objects.filter(credit_note=instance)
...
def delete_journal_voucher_against_tax_credit_note(sender, instance, **kwargs):
    ...
    """
    credit_gst = CreditNoteAccountTax.objects.filter(credit_note=instance)
    for s in credit_gst:
        s.save()
        JournalVoucher.objects.filter(
            user=s.credit_note.user, company=s.credit_note.company, voucher_id=s.id).delete()
