...
class Profile(models.Model):
	...
	def save(self, *args, **kwargs):
		if self.image:
			...
			imageTemproaryResized = imageTemproary.resize( (480,480) ) 
...
def product_activation(sender, instance, created, **kwargs):
	for product in instance.subscribed_products.all():
		if Product_activation.objects.filter(User=instance.Name,product=product).exists():
			...
			Product_activation.objects.update_or_create(User=instance.Name,date=timezone.localtime(),product=product,is_active=False)

class Role_product_activation(models.Model):
	User 	    = models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.CASCADE,null=True,blank=True)
	product     = models.ForeignKey(Role_based_product,on_delete=models.CASCADE,related_name='role_product_activate')
	date 		= models.DateField(default=datetime.date.today)
	is_active   = models.BooleanField(default=False)
...
def role_product_activation(sender, instance, created, **kwargs):
	for product in instance.subscribed_role_products.all():
		if Role_product_activation.objects.filter(User=instance.Name,product=product).exists():
			...
			Role_product_activation.objects.update_or_create(User=instance.Name,date=timezone.localtime(),product=product,is_active=False)
