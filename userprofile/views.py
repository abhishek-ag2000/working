...
class profiledetailview(LoginRequiredMixin,DetailView):
	...
	def get_context_data(self, **kwargs):
		...
		if self.request.user.is_authenticated:
			...
			context['services'] = Pro_services.objects.filter(User=self.request.user).order_by('-id')[:4]
			...
			context['Products_aggrement'] = Product_activation.objects.filter(product__id = 9, is_active=True)
			...
			context['services'] = Pro_services.objects.all().order_by('-id')[:4]
...
def specific_profile(request, pk):
	...
	if request.method == "POST":
		...
		if message_form_profile.is_valid():
			...
			answer = Message.objects.create(reciever=get_profile, sender=request.user, msg_content=msg_content,subject=subject)
			answer.save()
	...
	consultancy_count = consultancy_user.aggregate(the_sum=Coalesce(Count('id'), Value(0)))
	...
	services = Pro_services.objects.filter(User=profile_details.Name).order_by('-id')[:4]
...
@login_required
def activate_subscriptions_rolebasedproductlist(request, product_activation_id):
    product = Role_product_activation.objects.get(pk=product_activation_id)
    product.is_active = True
    product.save()
...
@login_required
def deactivate_subscriptions_rolebasedproductlist(request, product_activation_id):
    product = Role_product_activation.objects.get(pk=product_activation_id)
    product.is_active = False
    product.save()
...
class service_list_view(ListView):
	model = Pro_services
	template_name = 'services/service_list_details.html'


	def get_queryset(self):
		return Pro_services.objects.filter(User=self.request.user).order_by('-id')

	def get_context_data(self, **kwargs):
		context = super(service_list_view, self).get_context_data(**kwargs) 
		context['categories_list'] = categories.objects.all()
		context['service_list'] = Pro_services.objects.filter(User=self.request.user).order_by('-id')
		if self.request.user.is_authenticated:
			context['Products'] = Product_activation.objects.filter(User=self.request.user,product__id = 1, is_active=True)
			context['Role_products'] = Role_product_activation.objects.filter(User=self.request.user,product__id = 1, is_active=True)
			context['Todos'] = Todo.objects.filter(User=self.request.user, complete=False)
...
def service_specific_profile(request, pk):
	profile_details = get_object_or_404(Profile, pk=pk)
...
def search_professionals(request):
	...
	if query:
		result = user_profile.filter(Q(Name__username__icontains=query) | Q(E_mail__icontains=query) | Q(Full_Name__icontains=query) | Q(State__icontains=query))
	else:
...
@login_required()
def search_professionals_members(request):
	template = 'Organisation_member/professional_members.html'
...
def add_members(request, pk):
	...
	user_organisation.members.add(user_profile.Name)
	user_organisation.save()

	return redirect(reverse('userprofile:search_prof'))
